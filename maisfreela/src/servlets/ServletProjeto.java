		...
		switch (acao) {
			...
			case "darLanceAction":
				...
				NotifyController.enviarNotificacao("Lance dado", "Seu projeto: "
						...
						p.getTitulo()+"</a> recebeu um lance, clique no t�tulo do projeto para mais informa��es.", p.getEmpresario().getUsuario());
			...
			case "cancelarProjetoAction":
			  ...
			  + "Você receberá 40% do valor já pago.", projetDao.getUsuarioByProjeto(cancelar_proj));
				//
				request.getRequestDispatcher("/maisfreela/projeto.jsp").forward(request,response);
			break;
